...
extension BudgetByIndex on IsarCollection<Budget> {
  Future<Budget?> getByMonth(int month) {
    return getByIndex(r'month', [month]);
  }

  Budget? getByMonthSync(int month) {
    return getByIndexSync(r'month', [month]);
  }

  Future<bool> deleteByMonth(int month) {
    return deleteByIndex(r'month', [month]);
  }

  bool deleteByMonthSync(int month) {
    return deleteByIndexSync(r'month', [month]);
  }

  Future<List<Budget?>> getAllByMonth(List<int> monthValues) {
    final values = monthValues.map((e) => [e]).toList();
    return getAllByIndex(r'month', values);
  }

  List<Budget?> getAllByMonthSync(List<int> monthValues) {
    final values = monthValues.map((e) => [e]).toList();
    return getAllByIndexSync(r'month', values);
  }

  Future<int> deleteAllByMonth(List<int> monthValues) {
    final values = monthValues.map((e) => [e]).toList();
    return deleteAllByIndex(r'month', values);
  }

  int deleteAllByMonthSync(List<int> monthValues) {
    final values = monthValues.map((e) => [e]).toList();
    return deleteAllByIndexSync(r'month', values);
  }

  Future<Id> putByMonth(Budget object) {
    return putByIndex(r'month', object);
  }

  Id putByMonthSync(Budget object, {bool saveLinks = true}) {
    return putByIndexSync(r'month', object, saveLinks: saveLinks);
  }

  Future<List<Id>> putAllByMonth(List<Budget> objects) {
    return putAllByIndex(r'month', objects);
  }

  List<Id> putAllByMonthSync(List<Budget> objects, {bool saveLinks = true}) {
